...
___ max_sum_six_distances(a, position
    __ position == len(a) - 1:
        r_ a[position]
    ____
        max_forward _ min(len(a) - position, 6)
...
___ solution(A
    values _ [0] * len(A)
    values[len(A) - 1] _ A[len(A) - 1]
    ___ i __ range(len(A) - 2, -1, -1
...
___ find_max_between(values, start, length
    max _ values[start]
    upto _ min(start + length, len(values))
